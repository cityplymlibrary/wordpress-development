<?php
// ccp-lib-wp, jake deery, 2020

// init script
function ccp_lib_wp_init() { // def call above (0)
	// vars
...
// add css
function ccp_lib_wp_enqueue_styles() { // def call above (1)
	$theme = wp_get_theme(); // theme object for id ref
	// want to add new css?
	// 1. put css in ./css/
	// 2. copy a block below
	// 3. modify
	// 4. test
	...
	// 3. dequeue styles
	wp_deregister_style( 'wpbs-style' );
	wp_dequeue_style( 'wpbs-style' );
}

// add js
function ccp_lib_wp_enqueue_scripts() { // def call above (2)
...
add_action( 'init', 'ccp_lib_wp_init' ); // run on load
add_action( 'wp_enqueue_scripts', 'ccp_lib_wp_enqueue_styles' ); // load style data
add_action( 'wp_enqueue_scripts', 'ccp_lib_wp_enqueue_scripts' ); // load script data

//
// overrides

//
// override main classes
...
//
// override display post
function ccp_lib_wp_display_post($multiple_on_page) { ?>
	<article id="post-<?php the_ID(); ?>" <?php post_class("block"); ?> role="article">
		<header>
			<nav aria-label="breadcrumbs">
				<?php custom_breadcrumbs(); ?>
			</nav>
			<?php ccp_lib_wp_display_post_meta($multiple_on_page) ?>
...
//
// override display post meta
function ccp_lib_wp_display_post_meta($short=true) { ?>
	<ul class="meta text-muted list-inline">
		<?php if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) : ?>
		<li class="list-inline-item">
			<?php
				echo '<i class="fas fa-comment"></i> ';
				comments_popup_link(__( 'Leave a comment', "simple-bootstrap"), $sp . __( '1 Comment', "simple-bootstrap"), $sp . __( '% Comments', "simple-bootstrap"));
			?>
		</li>
		<?php endif; ?>
		<?php if (! $short) : ?>
...
//
// breadcrumbs code - thanks to thewebtaylor for kindly offering the codebase
