...
class MO_Control_Handler {
	...
	function testing_setup(){
		if(isset($_REQUEST['option']) && $_REQUEST['option']=='test_setup'){	?><script>
	...
	function testing_redirect(){
			...
			if(isset($_REQUEST['option']) && $_REQUEST['option'] == 'testing_started'){
	...
	function testing_oauthcallback(){
			if( isset( $_REQUEST['option'] ) and strpos( $_REQUEST['option'], 't_oauthredirect' ) !== false ) {
				...
				//==base64_encode("googleapps")
				if($_GET['state'] == base64_encode("googleapps")){
				...
				exit('Invalid response');
					...
					foreach($existing_applist as $key=>$app){
						...
						if($currentappname == $key){
							...
							$accessToken_test = $mo_oauth_handler->getAccessToken($accessTokenUrl_test, 'authorization_code', $current_app_data['clientid'],  $current_app_data['clientsecret'], $_GET['code'], $current_app_data['redirecturi']);
